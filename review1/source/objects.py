...
class Field:
	...
	def draw(self):
		...
		self.screen.addstr(0, 0, '┏' + (self.width - 3) * '━' + '┓')
		self.screen.addstr(self.height - 2, 0, '┗' + (self.width - 3) * '━' + '┛')
		self.screen.refresh()
...
class Snake:
	...
	def move(self, food, score):
		...
		head = self.queue[-1]
		newhead = ((self.field.height - 3 + head[0] + self.speed[0]) % (self.field.height - 3), (self.field.width - 3 + head[1] + self.speed[1]) % (self.field.width - 3))
